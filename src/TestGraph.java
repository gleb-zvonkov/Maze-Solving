...
public class TestGraph {
	...
	for (i = 0; i < 9; ++i)
	    for (j = 0; j < i; ++j)
		if (M[i][j] == 1) G.addEdge(V[i],V[j],"a");
		else if (M[i][j] == 2)G.addEdge(V[i],V[j],"b");
	}
	catch (GraphException e) {
        ...
        try {
            for (i = 0; i < numNodes; ++i) {
                ...
                while (neighbours.hasNext()) {
                    ...
                    else if ((uv.getBusLine().charAt(0) == 'a') && (M[node1][node2] != 1)) {
                        System.out.println("    Error: There should not be a bus line \"a\" between");
                        System.out.println("           nodes "+node1+" and "+node2);
                        failed = true;
                    }
                    else if ((uv.getBusLine().charAt(0) == 'b') &&(M[node1][node2] != 2)) {
