// This code was not created by me. This code was provided. 
...
public class DisplayMap extends JFrame {
  ...
  private String setB;         // Only one of these lines can be used in the solution
  ...
  private Color streetColor = Color.darkGray;
  private Color white = new Color(255,255,255);
  private Color black = new Color(0,0,0);
  private Color brown2 = new Color(188,120,120);  
  private Color brown = new Color(20,100,20);
  private Color toll = new Color(255,0,155);

  private Color[] lineColors = {Color.blue, Color.cyan, brown2, Color.magenta, 
						        Color.orange, Color.green, brown, Color.lightGray,
  ...
  /* ============================= */
    ...
    try {
      ...
      if (gridSize <= 10) legendHeight = 50;
      if (mapLength > 15) setSize((2*mapWidth-1)*gridSize+10,(2*mapLength+2)*gridSize+legendHeight);
      else if (mapLength >= 10) setSize((2*mapWidth-1)*gridSize+10,(2*mapLength+1)*gridSize+legendHeight);
      else setSize((2*mapWidth-1)*gridSize+10,(2*mapLength)*gridSize+legendHeight);
 ...
 /* Read the icons that will be displayed in the map */ 
 /* ================================================================= */
...
/* Assign a color to the bus line specified in the parameter */
 /* ================================================================= */
    private Color roadColor(char c) {
 /* ================================================================= */
	int i;

	if (Character.isLetter(c) == false) return streetColor;
	i = 0;
 ...
 /* ================================================================= */
	 ...
	 if ((row % 2) == 0) { // Horizontal street
           ...
           /* Draw an odd numbered row of the map */
	         switch (map[row][i]) {
           ...
           for (i = 0; i < (2*mapWidth-1); ++i)
           /* Draw an even numbered row of the map */
	         switch (map[row][i]) {
                  case '*': drawBlock(i,row); break;
		          default: color = roadColor(map[row][i]);
  ...
  /* Draw a legend at the botoom of the screen specifying the bus lines in set B */
  /* ====================== */
  private void drawLegend() {
  /* ====================== */
		String line;
		int posx = 10;
		int posy = (2*mapLength-1)*gridSize+10;
		if (setB == null || setB.length() == 0) return;
		StringTokenizer st = new StringTokenizer(setB);
		while (st.hasMoreTokens()) {
			line = st.nextToken();
			display.setColor(roadColor(line.charAt(0)));
			display.fillRect(posx,posy,20,20);
			posx = posx + 30;
		}
		display.setColor(Color.red);
        display.setFont(new Font("Dialog", Font.BOLD, 16));		
		display.drawString("Set B",posx+10,posy+18);
  }


/* Draw the dashed lines that appear in th emiddle of the roads */
 /* ================================================================= */
  ...
  /* ================================================================= */
      ...
      if (gridSize <= 2*dashSize) display.drawLine(x1,y1,x2,y2);
		...
		i = 0; 
        while (((y1 == y2) && ((x1+i) < x2)) || ((x1 == x2) && ((y1+i) < y2))) {
...
/* ================================================================= */
  ...
  /* ==================================== */
  public void drawLine (Node u, Node v) {
  /* ==================================== */
